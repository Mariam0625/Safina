...
export default function ResiliencePathway() {
  ...
  // Sample local data (can be replaced with real backend milestones later)
  ...
  const hasRecentLowMood = useMemo(() => milestones.some((m) => m.mood === "low"), [milestones]);
  ...
  // 🔗 When low mood exists, ask backend AI for a tiny localized micro-action
  useEffect(() => {
    if (!hasRecentLowMood) {
      setMicroAction(null);
      return;
    ...
    (async () => {
      try {
        const data = await api("/assistant/reply", {
          method: "POST",
          body: JSON.stringify({
            text: "I got a rejection and I'm feeling low", // Arabic users will get Arabic reply automatically
            context: { screen: "resilience" }
          })
        });
        if (isMounted) {
          setMicroAction(data?.micro_action || null);
        }
      } catch (e) {
        if (isMounted) setMicroAction(null);
      }
    })();
    return () => {
      isMounted = false;
    };
    // re-run when language toggles so the micro-action switches language
  }, [hasRecentLowMood, localStorage.getItem("language")]);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {/* Reframing Prompts (from AI when mood is low) */}
      {hasRecentLowMood && (
          ...
          <CardContent className="space-y-4">
            <div className="p-4 rounded-lg bg-muted/50">
              ...
              <p className="text-sm text-muted-foreground mb-3">
                {microAction ?? t("tweakCoverLetter")}
              </p>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>{/* Localize if you add a key later */}Your personalized journey map</CardDescription>
            </div>
              ...
              return (
                <div
                  ...
                  className={`p-4 rounded-lg border-2 text-center space-y-2 transition-smooth ${
                    badge.earned ? "border-secondary bg-secondary/10 shadow-soft" : "border-dashed border-muted bg-muted/20 opacity-50"
                  }`}
                >
                  <div
                  ...
                  {badge.earned && <Badge variant="secondary" className="text-xs">Earned</Badge>}
                </div>
              );
            })}
          </div>
